...
import { toDataRecoverySettings, toMuteList, toProfileEditor } from '@/lib/link'
...
import { Bitcoin, Check, Copy, Link, RotateCcw, Zap } from 'lucide-react'
...
export default function Profile({ id }: { id?: string }) {
  ...
  const isFollowingYou = useMemo(() => {
    return !!myPubkey && myPubkey !== profile?.pubkey && followings.includes(myPubkey)
  }, [followings, profile, myPubkey])
  const isSelf = myPubkey === profile?.pubkey
  ...
  useEffect(() => {
    ...
    const forceUpdateCache = async () => {
      await Promise.all([
        relayListService.forceUpdateRelayListEvent(profile.pubkey),
        profileFetcher.fetchProfile(profile.pubkey, true)
      ])
    }
    forceUpdateCache()
  }, [profile?.pubkey])
